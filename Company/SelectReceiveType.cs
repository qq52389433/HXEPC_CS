...
{
    public class SelectReceiveType
    {
        //获取文件类型
        public static JObject GetSelectReceiveTypeList(string sid, string ProjectKeyword, string page, string Filter)
        {
            ...
            try
            {
                ...
                //string curReceiveTypeCode = "";

                //curReceiveTypeCode = prjProject.Code;//.GetValueByKeyWord("PRO_COMPANY");
                //if (string.IsNullOrEmpty(curReceiveTypeCode))
                //{
                //    reJo.msg = "获取项目来源失败！";
                //    return reJo.Value;
                //}
                ...
                //获取所有参建单位
                List<DictData> dictDataList = dbsource.GetDictDataList("DocumentType");
                ...
                foreach (DictData data6 in resDDList)
                    {
                        //if (data6.O_sValue1 == curReceiveTypeCode)
                        {
                            JObject joData = new JObject(
                                new JProperty("receiveTypeType", "文件类型"),
                                new JProperty("receiveTypeId", data6.O_ID.ToString()),
                                new JProperty("receiveTypeCode", data6.O_Code),
                                new JProperty("receiveTypeDesc", data6.O_Desc)
                                );
                            jaData.Add(joData);
                        }
                    }
                    ...
                    //    {
                    //        JObject joData = new JObject(
                    //            new JProperty("receiveTypeType", "项目部门"),
                    //            new JProperty("receiveTypeId", data6.O_ID.ToString()),
                    //            new JProperty("receiveTypeCode", data6.O_Code),
                    //            new JProperty("receiveTypeDesc", data6.O_Desc)
                    //            );
                    ...
                    //            new JProperty("receiveTypeType", "项目部门"),
                    //            new JProperty("receiveTypeId", data6.O_ID.ToString()),
                    //            new JProperty("receiveTypeCode", data6.O_sValue1),
                    //            new JProperty("receiveTypeDesc", data6.O_Desc)
                    //            );
                    //        jaData.Add(joData);
                    //    }
